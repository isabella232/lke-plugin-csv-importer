...
function addEdgeProperty(edge) {
  ...
  let deleteEdgePropertyButton = property.getElementsByClassName(
    'removeedgepropertybutton'
  )[0];
  deleteEdgePropertyButton.id =
    'removeedgepropertybutton-' + property.id.split('-')[1];
...
function addIdentifier(node) {
  ...
  let deleteIdentifierButton = identifier.getElementsByClassName(
  ...
  if (identifier.id.startsWith('from')) {
    deleteIdentifierButton.id =
      'removefromidentifierbutton-' + identifier.id.split('-')[1];
    fillOptionsNode(selectProperty, propertiesFrom);
  } else {
    deleteIdentifierButton.id =
      'removetoidentifierbutton-' + identifier.id.split('-')[1];
...
async function getProperties(categoryName, entityType) {
  ...
  const selectedCategory = schema.find(
    (category) =>
      category.access === 'write' && category.itemType === categoryName
  );
  if (selectedCategory) {
...
function getNodesData() {
  ...
  for (let n = 0; n < nodes.length; n++) {
    ...
    const nodeName = nodeSelect.length
      ? nodeSelect[0].value
      : node.getElementsByClassName('label')[0].value;
    ...
    for (let p = 0; p < properties.length; p++) {
      ...
      const propertyName = propertySelect.length
        ? propertySelect[0].value
        : property.getElementsByClassName('property')[0].value;
  ...
  for (let n = 0; n < nodes.length; n++) {
    ...
    for (let p = 0; p < properties.length; p++) {
      ...
      query +=
        ...
        propertyName +
        ' = ~' +
        headers.options[headers.selectedIndex].value +
        '~ ';
    }
...
function getEdgesData() {
  ...
  for (let e = 0; e < tables.length; e++) {
    ...
    edge.from.category = fromNode.getElementsByClassName(
      'node_select'
    )[0].value;
    ...
    for (let p = 0; p < fromIdentifiers.length; p++) {
      ...
      const indexColumn = headers.options[headers.selectedIndex].value;
      const identifierName = identifier.getElementsByClassName(
        'property_select'
      )[0].value;
    ...
    edge.edge.type = edgeType.length
      ? edgeType[0].value
      : edgeElement.getElementsByClassName('label')[0].value;
    let properties = edgeElement.getElementsByClassName('propertyClass');
    for (let p = 0; p < properties.length; p++) {
      let property = properties[p];
      let headers = property.getElementsByClassName('headers')[0];
      const indexColumn = headers.options[headers.selectedIndex].value;
      const edgePropertySelect = property.getElementsByClassName(
        'property_select'
      );
      const propertyName = edgePropertySelect.length
        ? edgePropertySelect[0].value
        : property.getElementsByClassName('edgeproperty')[0].value;
...
function createEdgeQuery() {
  ...
  for (let e = 0; e < tables.length; e++) {
    ...
    let fromQuery =
      'MATCH (f:' +
      fromNode.getElementsByClassName('node_select')[0].value +
      ') ';
    for (let p = 0; p < fromIdentifiers.length; p++) {
      let identifier = fromIdentifiers[p];
      let headers = identifier.getElementsByClassName('headers')[0];
      ...
      fromQuery +=
        identifier.getElementsByClassName('property_select')[0].value +
        ' = ~' +
        headers.options[headers.selectedIndex].value +
        '~ ';
    }
    ...
    const edgeTypeFinal = edgeType.length
      ? edgeType[0].value
      : edge.getElementsByClassName('label')[0].value;
    ...
    let toQuery =
      'MATCH (t:' +
      toNode.getElementsByClassName('node_select')[0].value +
      ') ';
...
function checkInput() {
  ...
  let node_selects = Array.from(
    document.getElementsByClassName('node_select')
  ).concat(edge_selects);
  let all_selects = Array.from(
    document.getElementsByClassName('property_select')
  ).concat(node_selects);
  ...
  for (let i = 0; i < tables.length; i++) {
    let fromid = tables[i]
      .getElementsByClassName('fromname')[0]
      .getElementsByClassName('identifierClass')[0]
      .getElementsByTagName('select')[0];
    let toid = tables[i]
      .getElementsByClassName('toname')[0]
      .getElementsByClassName('identifierClass')[0]
      .getElementsByTagName('select')[0];
